...
export const metadata: Metadata = {
  title: "The Course",
  description: "Explore all 18 holes of Ashfield Golf Club's parkland course in South Armagh. 5,620 yards, par 69, GUI affiliated. Each hole carries a name rooted in the local landscape.",
};
      ...
      <div style={{ background: "#f5f0e8", padding: "3rem 2rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1rem", marginBottom: "2.5rem" }}>
            {HOLES.map((h) => (
              <div key={h.number} style={{ background: "#fff", border: "1px solid rgba(26,58,42,0.1)", borderLeft: `3px solid ${h.white.par === 3 ? "#c9a84c" : h.white.par === 5 ? "#4a8c62" : "#2d5a3f"}`, padding: "1.25rem 1.5rem", borderRadius: 2 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <span style={{ fontSize: 11, color: "#6b7c68", textTransform: "uppercase", letterSpacing: "1px" }}>Hole {h.number}</span>
                    <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: "#1a3a2a", marginTop: "0.15rem" }}>{h.name}</h3>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, color: "#2d5a3f", display: "block" }}>Par {h.white.par}</span>
                    <span style={{ fontSize: 12, color: "#6b7c68" }}>SI {h.white.si}</span>
                  </div>
                </div>
                <div style={{ marginTop: "0.75rem", display: "flex", gap: "1.5rem" }}>
                  <div><span style={{ fontSize: 11, color: "#6b7c68", display: "block" }}>Men</span><span style={{ fontSize: 14, color: "#1a2218", fontWeight: 500 }}>{h.white.yards} yds</span></div>
                  <div><span style={{ fontSize: 11, color: "#6b7c68", display: "block" }}>Ladies</span><span style={{ fontSize: 14, color: "#1a2218", fontWeight: 500 }}>{h.ladies.yards} yds</span></div>
                </div>
              </div>
            ))}
          </div>
