...
export default function GalleryStrip() {
  const images = [
    ...
    { src: "/images/course/ball-on-timber-sleeper.webp", alt: "A golf ball resting on a weathered timber sleeper beside the fairway" },
    ...
    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 4, height: "min(280px, 40vw)" }}>
      {images.map((img, i) => (
        <div key={i} style={{ overflow: "hidden", background: "#2d5a3f", position: "relative" }}>
          <img src={img.src} alt={img.alt} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.85, transition: "opacity 0.3s, transform 0.4s" }}
